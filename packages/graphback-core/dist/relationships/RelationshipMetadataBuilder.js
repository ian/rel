...
exports.RelationshipMetadataBuilder = void 0;
/* eslint-disable max-lines */
const graphql_1 = require("graphql");
const graphql_metadata_1 = require("graphql-metadata");
const crud_1 = require("../crud");
const db_1 = require("../db");
const hasListType_1 = require("../utils/hasListType");
 ...
 */
class RelationshipMetadataBuilder {
    ...
    buildModelRelationshipContext(modelType) {
        ...
        for (let field of fields) {
            const annotation = relationshipHelpers_1.parseRelationshipAnnotation(field.description);
            if (!annotation) {
                continue;
            }
            this.validateRelationshipField(modelType.name, field, annotation);
            const relationType = graphql_1.getNamedType(field.type);
            ...
            if (annotation.kind === 'oneToMany') {
                ...
                const oneToManyAnnotation = annotation.key ? annotation : relationshipHelpers_1.parseRelationshipAnnotation(field.description);
                const manyToOneAnnotation = relationshipHelpers_1.parseRelationshipAnnotation(relationField.description);
                ...
                const oneToManyAnnotation = relationshipHelpers_1.parseRelationshipAnnotation(relationField.description);
                ...
                // avoid reparsing if annotation key defined
                const oneToOneAnnotation = annotation.key ? annotation : relationshipHelpers_1.parseRelationshipAnnotation(field.description);
                this.addOneToOne(modelType, field, oneToOneAnnotation);
            }
        }
    }
    createOneToManyField(fieldName, baseType, relationFieldName, columnName) {
        const columnField = columnName || db_1.transformForeignKeyName(relationFieldName);
        const fieldDescription = relationshipHelpers_1.relationshipFieldDescriptionTemplate('oneToMany', relationFieldName, columnField);
        const fieldType = graphql_1.GraphQLNonNull(graphql_1.GraphQLList(baseType));
        return {
            name: fieldName,
    ...
    createManyToOneField(fieldName, baseType, relationFieldName, columnName) {
        const columnField = columnName || db_1.transformForeignKeyName(fieldName);
        const fieldDescription = relationshipHelpers_1.relationshipFieldDescriptionTemplate('manyToOne', relationFieldName, columnField);
        return {
            name: fieldName,
            description: fieldDescription,
            type: baseType,
    ...
    updateOneToManyField(field, relationFieldName, columnName) {
        if (!columnName) {
            const columnField = db_1.transformForeignKeyName(relationFieldName);
            const fieldDescription = relationshipHelpers_1.relationshipFieldDescriptionTemplate('oneToMany', relationFieldName, columnField);
            const oldDescription = field.description ? `\n${field.description}` : '';
            return Object.assign(Object.assign({}, field), { description: `${fieldDescription}${oldDescription}` });
        }
        return field;
    }
    updateManyToOneField(field, relationFieldName, columnName) {
        const manyToOneMetadata = graphql_metadata_1.parseMetadata("manyToOne", field.description);
        if (!manyToOneMetadata || !manyToOneMetadata.key) {
            const columnField = columnName || db_1.transformForeignKeyName(field.name);
            const fieldDescription = relationshipHelpers_1.relationshipFieldDescriptionTemplate('manyToOne', relationFieldName, columnField);
            const oldDescription = field.description ? `\n${field.description}` : '';
            return Object.assign(Object.assign({}, field), { description: `${fieldDescription}${oldDescription}` });
        }
    ...
    updateOneToOneField(field, columnName) {
        if (!columnName) {
            const columnField = db_1.transformForeignKeyName(field.name);
            const fieldDescription = relationshipHelpers_1.relationshipOneToOneFieldDescriptionTemplate('oneToOne', columnField);
            const oldDescription = field.description ? `\n${field.description}` : '';
            return Object.assign(Object.assign({}, field), { description: `${fieldDescription}${oldDescription}` });
        }
        return field;
    ...
    validateOneToManyRelationship(modelName, field, oneToManyMetadata, corresspondingManyToOneMetadata) {
        ...
        const relationModelType = graphql_1.getNamedType(field.type);
        const relationField = relationModelType.getFields()[oneToManyMetadata.field];
        // field will be generated, no need to validate
        if (!relationField) {
            return;
        }
        if (hasListType_1.hasListType(relationField.type)) {
            throw new Error(`${relationModelType.name}.${relationField.name} is a list type, but should be '${relationField.name}: ${modelName}'.`);
        }
        const relationFieldBaseType = graphql_1.getNamedType(relationField.type);
        if (!graphql_1.isObjectType(relationFieldBaseType) || relationFieldBaseType.name !== modelName) {
        ...
        if ((oneToManyMetadata === null || oneToManyMetadata === void 0 ? void 0 : oneToManyMetadata.key) !== (corresspondingManyToOneMetadata === null || corresspondingManyToOneMetadata === void 0 ? void 0 : corresspondingManyToOneMetadata.key)) {
    ...
    validateOneToOneRelationship(modelName, field, oneToOneAnnotation) {
        ...
        if (hasListType_1.hasListType(field.type)) {
    ...
    validateRelationshipField(modelName, field, relationshipAnnotation) {
        ...
        const fieldBaseType = graphql_1.getNamedType(field.type);
        if (!graphql_1.isObjectType(fieldBaseType)) {
        ...
        if (!crud_1.isModelType(fieldBaseType)) {
...
exports.RelationshipMetadataBuilder = RelationshipMetadataBuilder;
