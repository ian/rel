...
import { cypherUpdate } from "../src/update"
...
describe("#cypherUpdate", () => {
  describe("basic creation", () => {
    it("should create a node with the properties specified", async () => {
      ...
      node = await cypherUpdate("Movie", node.id, { year: 1999 })
  ...
  describe("opts", () => {
    describe("id", () => {
      it("should not change the id", async () => {
        ...
        node = await cypherUpdate("Movie", node.id, { title: "The Matrix" })
    ...
    describe("timestamps", () => {
      it("should change the updatedAt but not createdAt by default", async () => {
        ...
        const node = await cypherUpdate("Movie", id, {
