...
describe("#model create", () => {
  ...
  describe("runtime", () => {
    ...
    it("should create the Book", async (done) => {
    ...
    describe("defaults", () => {
      it("should default to static value", async (done) => {
        const { graphql } = server(
          Rel.model("Book", {
            title: Rel.string().default("STATIC"),
          }).endpoints(true)
        )
        ...
        expect(data?.book.title).toEqual("STATIC")
        expect(errors).toBeUndefined()

        done()
      })

      it("should default to function value", async (done) => {
        const { graphql } = server(
          Rel.model("Book", {
            title: Rel.string().default(() => "FUNCTION"),
          }).endpoints(true)
        )
        ...
        expect(data?.book.title).toEqual("FUNCTION")
        expect(errors).toBeUndefined()
